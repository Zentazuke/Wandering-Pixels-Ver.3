/**
 * exportBoard.js — Canvas-render the board to a PNG download.
 * Ported directly from the old board/export.js with minor cleanups.
 */
 ...
 * @param {string}   currentBg
 * @param {string|null} customBgColor
 * @param {string|null} customBgImage
 * @param {boolean}  returnDataUrl  true = return jpeg thumbnail, false = download PNG
 */
export async function exportBoard(elements, currentBg, customBgColor, customBgImage, returnDataUrl = false) {
  ...
  for (const el of sorted) {
    ...
    if (el.type === 'photo' && el.src) {
      await new Promise((res) => {
        ...
        img.onload = () => {
          const FRAME_REMAP = { dark:'polaroid', navy:'polaroid', sage:'polaroid', rose:'polaroid', kraft:'vintage', black:'thick', rounded:'none', round14:'none' };
    ...
    // ── Text ──
      ...
      const FRAME_BG = { polaroid:'#fff','dark-card':'#1a1712',crt:'#0a1a0a',blueprint:'#0d2a4a',kraft:'#c8913a' };
