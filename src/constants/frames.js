/** Photo frame styles — decorative border around the photo element.
 *  pt         → padding (CSS shorthand string)
 *  bg         → frame background colour
 *  br         → border-radius
 *  capColor   → caption text colour  ('' = no caption)
 *  capFont    → caption font-family  ('' = no caption)
 *  wrapStyle  → extra inline CSS applied to the outer wrapper element
 *  overlay    → raw HTML string injected as a child overlay (SVG effects, dot patterns, etc.)
 *  double     → true = render a second inner border
 *  shadowOnly → true = no visible frame, just a drop shadow
 */
export const FRAMES = {
