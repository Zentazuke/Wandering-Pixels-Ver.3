...
// Frame styles — same as old NOTE_FRAME_STYLES
const NOTE_FRAME_STYLES = {
  none:        '',
...
const FRAME_BG_OVERRIDES = {
  polaroid: '#fff', 'dark-card': '#1a1712', crt: '#0a1a0a', blueprint: '#0d2a4a', kraft: '#c8913a',
};

// Parse a camelCase-style shorthand string into a style object
function parseFrameStyle(str) {
  const obj = {};
...
const TextElement = memo(function TextElement({ el, onPointerDown, onRotate, onResize }) {
  ...
  const onKeyDown = useCallback((e) => {
    ...
    // Enter inside <li> → insert new bullet
  ...
  return (
    ...
    >
      {/* Tape decoration for tape-top frame */}
      {frameKey === 'tape-top' && (
      ...
      <div
        ...
        style={{ background: wrapBg, minHeight: el.h, ...frameStyle }}
      >
        {/* Speech bubble tail */}
        {frameKey === 'speech' && (
