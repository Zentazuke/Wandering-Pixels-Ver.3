...
export default function PhotoProps({ el }) {
  ...
  const upd = (patch) => update(el.id, patch);
        ...
        <div className={styles.shapeGrid}>
          {Object.entries(SHAPES).map(([sid, sh]) => (
            <ShapeBtn
              ...
              sid={sid}
              sh={sh}
              active={activeShape === sid}
              onClick={() => upd({ shape: sid })}
            />
          ))}
        </div>
      </PropSection>
...
// ── Shape button ──────────────────────────────────────────────────────────────
function ShapeBtn({ sid, sh, active, onClick }) {
  return (
